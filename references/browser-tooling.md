...
| Video / trace / HAR | `record`, `trace`, `network har` | gif_creator only | screencast via CDP |
| Scriptable from a CLI | yes (the whole point) | no, MCP tool calls only | yes |

Why it beats Chrome MCP for verification: every step is a shell command, so it composes in scripts (`smoke`), runs in parallel sessions, does not touch the user's own tabs, and costs a fraction of the tokens (ref-based snapshots instead of screenshots-as-context). Chrome MCP stays useful when you need the user's logged-in browser (e.g. the Shopify admin UI).
...
- `record start file.webm` creates a fresh context but keeps cookies.
